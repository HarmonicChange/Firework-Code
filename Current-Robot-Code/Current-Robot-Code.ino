...
void loop() {
  ...
  if (lineLeft > blackDetect && lineRight > blackDetect && intRdy) {
    ...
    Direction temp;
    if (currDir-1 <0) temp = Direction(currDir-1+4);
      else temp = Direction((currDir-1)%4);
    if (isThereAWall(0)==1){ //Left wall present
      grid[currPos]->addWall(Direction(temp), true);
      Serial.println("Leftwall detected");
    } else {
      grid[currPos]->addWall(Direction(temp), false);
...
//Return if there is a wall Threshold
